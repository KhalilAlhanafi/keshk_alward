                                    ...
                                    <div class="mt-2 flex flex-wrap gap-4">
                                        @if($order->payment_proof)
                                            <div class="bg-white/70 p-2.5 rounded-xl border border-amber-200 flex flex-col gap-1">
                                                ...
                                                <button 
                                                    type="button" 
                                                    @click="openImageModal('{{ str_starts_with($order->payment_proof, 'data:image/') ? $order->payment_proof : asset('storage/' . ltrim($order->payment_proof, '/')) }}', 'إيصال دفع شام كاش — طلب {{ $order->order_number }}')" 
